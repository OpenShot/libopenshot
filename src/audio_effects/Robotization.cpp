...
// Default constructor
Robotization::Robotization(openshot::FFTSize new_fft_size, openshot::HopSize new_hop_size, openshot::WindowType new_window_type, openshot::RobotizationEffectType new_effect_type) : 
			 fft_size(new_fft_size), hop_size(new_hop_size), window_type(new_window_type), effect_type(new_effect_type), stft(*this)
{
	// Init effect properties
	init_effect_details();
}
...
void Robotization::SetJsonValue(const Json::Value root) {
	...
	if (!root["effect_type"].isNull())
		effect_type = (RobotizationEffectType)root["effect_type"].asInt();
...
std::string Robotization::PropertiesJSON(int64_t requested_frame) const {
	...
	root["effect_type"] = add_property_json("Effect Type", effect_type, "int", "", NULL, 0, 2, false, requested_frame);
    ...
    // Add effect_type choices (dropdown style)
	root["effect_type"]["choices"].append(add_property_choice_json("Pass Through", PASS_THROUGH, effect_type));
	root["effect_type"]["choices"].append(add_property_choice_json("Robotization", ROBOTIZATION, effect_type));
	root["effect_type"]["choices"].append(add_property_choice_json("Whisperization", WHISPERIZATION, effect_type));
